// src/components/investigadores/PerfilInvestigador.tsx
'use client'
...
import styles from './PerfilInvestigador.module.css'
...
export default function PerfilInvestigador({ id, onVolver }: Props) {
  const { data: investigador, isLoading } = useQuery<Investigador>({
    queryKey: ['investigador', id],
    queryFn:  () => GET(`/core/investigador/${id}`),
  })

  if (isLoading) return (
    <div className={styles.estadoCentro}>
      <StateView variant="loading" title="Cargando perfil" message="Preparando la ficha del investigador..." />
    </div>
  )
  if (!investigador) return (
    <div className={styles.estadoCentro}>
      <StateView variant="empty" title="Investigador no encontrado" message="No encontramos información para este investigador." />
    </div>
  )

  const nombre = nombreCompleto(investigador.nombre_completo,
  //  investigador.apellido_paterno, investigador.apellido_materno
  )
  //const edad   = investigador.fecha_nacimiento ? calcularEdad(investigador.fecha_nacimiento) : null
  ...
  return (
    <div className={styles.page}>
        ...
        <div className={styles.heroBody}>
          ...
          <div className={styles.heroInfo}>
            ...
            <div className={styles.meta}>
              ...
              {investigador.genero  && <span className={styles.metaItem}>👤 {investigador.genero}</span>}
              {investigador.municipio_nombre && <span className={styles.metaItem}>📍 {investigador.municipio_nombre}</span>}
              {investigador.localidad_nombre && <span className={styles.metaItem}>🏠 {investigador.localidad_nombre}</span>*/}
            </div>
          </div>
        </div>
      </div>
      ...
      <div className={styles.statsGrid}>
        <StatPerfil emoji="🩺" label="ID"          valor={String(investigador.id)} />
        <StatPerfil emoji="🌽" label="Experiencia"
          valor={investigador.anios_experiencia
            ? `${investigador.anios_experiencia} ${investigador.anios_experiencia === 1 ? 'año' : 'años'}`
            : undefined} />
        <StatPerfil emoji="📍" label="Comunidad"   valor={investigador.comunidad_nombre} />
        <StatPerfil emoji="📅" label="Fecha nac." valor={investigador.fecha_nacimiento ?? undefined} />
      </div>
