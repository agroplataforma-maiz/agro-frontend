...
import styles from './PerfilTecnico.module.css'
...
export default function PerfilTecnico({ id, onVolver }: Props) {
  const { data: tecnico, isLoading } = useQuery<TecnicoCampo>({
    queryKey: ['tecnico', id],
    queryFn:  () => GET(`/core/tecnico/${id}`),
  })

  if (isLoading) return (
    <div className={styles.estadoCentro}>
      <StateView variant="loading" title="Cargando perfil" message="Preparando la ficha del técnico..." />
    </div>
  )
  if (!tecnico) return (
    <div className={styles.estadoCentro}>
      <StateView variant="empty" title="Técnico no encontrado" message="No encontramos información para este técnico." />
    </div>
  )
    ...
    <div className={styles.page}>
      ...
      <div className={styles.statsGrid}>
        ...
        <StatPerfil emoji="🌽" label="Experiencia"
          valor={tecnico.anios_experiencia
            ? `${tecnico.anios_experiencia} ${tecnico.anios_experiencia === 1 ? 'año' : 'años'}`
            : undefined} />
        <StatPerfil emoji="📍" label="Comunidad"   valor={tecnico.comunidad_nombre} />
        <StatPerfil emoji="📅" label="Fecha nac." valor={tecnico.fecha_nacimiento ?? undefined} />
      </div>
